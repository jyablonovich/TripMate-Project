  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = "arn:aws:sns:*:*:${var.project}-viaje-*"
      }
    ]
  })
}
...
resource "aws_lambda_function" "backend" {
  ...
  environment {
    variables = {
      ...
      TOPIC_PREFIX = "${var.project}-viaje-"
    }
  }
...
resource "aws_api_gateway_integration" "int_callback" { # <— EXISTE
  rest_api_id             = aws_api_gateway_rest_api.api.id
  resource_id             = aws_api_gateway_resource.res_callback.id
...
resource "aws_api_gateway_integration" "int_signout" { # <— EXISTE
  rest_api_id             = aws_api_gateway_rest_api.api.id
  resource_id             = aws_api_gateway_resource.res_signout.id
...
resource "aws_api_gateway_integration" "int_get_actividades" { # nombre único
  rest_api_id             = aws_api_gateway_rest_api.api.id
  resource_id             = aws_api_gateway_resource.res_actividades.id
...
resource "aws_api_gateway_integration" "int_post_actividades" { # nombre único
  rest_api_id             = aws_api_gateway_rest_api.api.id
  resource_id             = aws_api_gateway_resource.res_actividades.id
...
resource "aws_api_gateway_integration" "int_post_votar" { # nombre único
  rest_api_id             = aws_api_gateway_rest_api.api.id
  resource_id             = aws_api_gateway_resource.res_votar.id
...
resource "aws_api_gateway_deployment" "deploy" {
  ...
  depends_on = [
    # integraciones declaradas arriba (¡todas existen!)
...
# Debe existir ya este recurso en lambdas_api
